...
// AS subclasses call super() (which compiles to r:super[undefined]())
// in the constructor. @babel/parser fails if we try to call super()
// outside a class, so instead we have to name it something else.
const SUPER_FN_NAME = "_super";
  ...
  generators = {
    FunctionDeclaration: (node) => {
      ...
      const registers = {
        ...
        meta: {
          this: registerAllocator.assign(1, "this"),
        },
        args: {},
        locals: {},
      };

      // If _super() is called, reserve super
      // super() must be the first statement, so only set the super register
      // if it is found as the first statement. This way we can just throw an
      // error if somebody tries to call _super() but registers.meta.super is
      // missing.
      const firstNode = node.body.body[0];
      if (
        firstNode?.type === "ExpressionStatement" &&
        firstNode?.expression?.type === "CallExpression" &&
        firstNode?.expression?.callee?.name === SUPER_FN_NAME
      ) {
        registers.meta.super = registerAllocator.assign(2, "super");
      }
      ...
      // Keep track of whether _super() has already been called so we can throw
          ...
          [
            this.contexts.function,
            {
              callSuper: (node) => {
                if (superCalled) {
                  throw new CompilerError(
                    `_super() cannot be called twice!`,
                    node
                  );
                }

                if (!registers.meta.super) {
                  throw new CompilerError(
                    `${SUPER_FN_NAME}() must be the first statement in a function, can cannot be called inside conditionals or loops!`,
                    node
                  );
                }

                // super is called like `super[undefined]()`
                this.emit(`push 0, ${registers.meta.super.toToken()}, UNDEF`);
                this.emit("callMethod");
                // don't pop, CallExpression should return it onto the stack
                superCalled = true;
              },
              declareVariable: (id) => {
    ...
    CallExpression: (node) => {
      ...
      if (callee.type === "Identifier" && callee.name === SUPER_FN_NAME) {
        const fnCtx = this.contexts.function.peek();
        if (!fnCtx) {
          throw new CompilerError(
            `${SUPER_FN_NAME} can only be called inside a function`,
            node
          );
        }

        fnCtx.callSuper(node);
        return;
      }
