...
class Contexts {
  ...
  withFunctionContext(ctx, fn) {
    return this.withContext(this._functionContext, ctx, fn);
  }

  peekFunctionContext() {
    return this.peek(this._functionContext);
  }

  // store context for loops here, such as emitBreak()
  withLoopContext(ctx, fn) {
    return this.withContext(this._loopContext, ctx, fn);
  }

  peekLoopContext() {
    return this.peek(this._loopContext);
  }

  withContext(context, ctxVal, fn) {
    context.push(ctxVal);
    const ret = fn();
    this.assertNotPromise(ret);
    context.pop();
  }
  ...
  assertNotPromise(val) {
    if (
      val &&
      typeof val.then === "function" &&
      val[Symbol.toStringTag] === "Promise"
    ) {
      throw new Error(`async functions cannot be used in withFunctionContext`);
    }
  }
}
...
class Compiler {
  ...
  generators = {
    FunctionDeclaration: (node) => {
      ...
      this._contexts.withFunctionContext(
        {
          declareVariable: (id) => {
    ...
    VariableDeclarator: (node) => {
      ...
      const fnCtx = this._contexts.peekFunctionContext();
      if (fnCtx) {
    ...
    WhileStatement: (node) => {
      ...
      this._contexts.withLoopContext(
        {
          emitBreak: () => {
    ...
    BreakStatement: (node) => {
      ...
      const loopCtx = this._contexts.peekLoopContext();
      if (!loopCtx) {
    ...
    AssignmentExpression: (node) => {
      ...
      const ctx = this._contexts.peekFunctionContext();
    ...
    MemberExpression: (node) => {
      ...
      const pushObjectToStack = () => {
        switch (object.type) {
          case "Identifier": {
            const objectInRegister = this._contexts
              .peekFunctionContext()
